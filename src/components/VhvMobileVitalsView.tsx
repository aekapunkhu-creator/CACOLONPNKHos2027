...
import { PHON_NA_KAEO_VILLAGES, SUBDISTRICT_LIST } from '../data/villages';
...
export const VhvMobileVitalsView: React.FC<VhvMobileVitalsViewProps> = ({
  ...
  const handleSaveVitals = (e: React.FormEvent) => {
    ...
    setNotification({
      type: 'success',
      message: `บันทึกข้อมูลสุขภาพของ ${updated.prefix}${updated.firstName} ${updated.lastName} เรียบร้อยแล้ว!`
    });
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 pb-16">
      ...
      <header className="sticky top-0 z-30 bg-gradient-to-r from-emerald-800 to-teal-900 text-white shadow-md">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center justify-between">
          ...
          {onExitVhvMode && (
            <button
              type="button"
              onClick={onExitVhvMode}
              className="text-xs text-emerald-200 hover:text-white px-2.5 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-colors flex items-center gap-1"
            >
      ...
      <main className="max-w-md mx-auto p-4 space-y-4">
        {/* Notification Banner */}
        {notification && (
          <div className={`p-3.5 rounded-2xl flex items-center gap-2.5 text-xs font-semibold shadow-xs ${
        ...
        <button
          ...
          className="w-full py-3.5 px-4 bg-gradient-to-r from-emerald-600 to-teal-600 active:from-emerald-700 active:to-teal-700 text-white font-bold text-sm rounded-2xl shadow-md flex items-center justify-center gap-2.5 transition-all"
        >
            ...
            <form onSubmit={handleSaveVitals} className="space-y-3.5">
              ...
              <div className="pt-2 flex gap-2">
                <button
                  type="button"
                  onClick={() => setSelectedPatientId(null)}
                  className="flex-1 py-3 border border-slate-300 text-slate-700 rounded-xl text-xs font-bold hover:bg-slate-50"
                >
                  ยกเลิก
                </button>
                <button
                  type="submit"
                  className="flex-2 py-3 bg-emerald-600 active:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-xs flex items-center justify-center gap-1.5"
                >
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                ...
                <span>หรือค้นหาผู้ป่วยในพื้นที่</span>
              </h2>
              ...
              {filteredPatients.map(p => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-xs font-semibold shadow-xs"
                    >
                      บันทึก
                    </button>
                  </div>
        ...
        <div className="p-3 rounded-xl bg-slate-200/60 text-slate-500 text-[11px] text-center flex items-center justify-center gap-1.5">
          ...
          <span>หน้าจอสำหรับ อสม. บันทึกข้อมูลสุขภาพ ไม่ต้องเข้าสู่ระบบ</span>
        </div>
      </main>
